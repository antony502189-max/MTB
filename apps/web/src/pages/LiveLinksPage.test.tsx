        ...
        data: {
          active_boosters: [
            ...
            },
            {
              ...
              boost_rate: 12,
              category: "electronics",
              end_at: "2026-04-23T11:00:00.000Z",
...
describe("LiveLinksPage", () => {
  it("renders active boosters and reward ledger entries", () => {
    ...
    expect(html.match(/электроника/g)?.length).toBe(1);
  });

  it("does not render hero metric cards for ledger count and network status", () => {
    const html = renderLiveLinksPage();

    expect(html).not.toContain("<span>Записи журнала</span>");
    expect(html).not.toContain("<span>Состояние</span>");
    expect(html).not.toContain("<strong>Онлайн</strong>");
  });
});
